...
import { getGhiSourceLabel, type GhiLookupResult } from "@/lib/solar/ghiData";
import { Loader2, Sun } from "lucide-react";

interface GhiPreviewBadgeProps {
  lookup: GhiLookupResult | null;
  loading?: boolean;
}

export function GhiPreviewBadge({ lookup, loading }: GhiPreviewBadgeProps) {
  if (loading) {
    return (
  ...
  const sourceColors: Record<GhiLookupResult["source"], string> = {
    cidade: "border-emerald-500/30 bg-emerald-500/8",
    estado: "border-amber-500/30 bg-amber-500/8",
    hsp_fallback: "border-amber-500/30 bg-amber-500/8",
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="font-semibold text-navy-900">
          GHI:{" "}
          {lookup.ghi.toLocaleString("pt-BR", {
        ...
        <span className="rounded-full bg-white/80 px-2 py-0.5 text-xs font-medium text-navy-700/70">
          {getGhiSourceLabel(lookup.source)}
        </span>
      </div>
